...
const (
	use   = "discli"
	short = "discli is a command line tool for managing container images in distribution"
)
...
func New(use, short string, options []crane.Option) *cobra.Command {
	...
	root.AddCommand(
		NewCmdAuth(options, "discli", "auth"),
		NewCmdCatalog(&options, "discli"),
		NewCmdCopy(&options),
		NewCmdDelete(&options),
		NewCmdList(&options),
		NewCmdPull(&options),
		NewCmdPush(&options),
		NewCmdCleanup(&options),
		NewCmdVersion(),
	)
